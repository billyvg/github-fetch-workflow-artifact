...
const actions = {
  ...
  listRepoWorkflows: jest.fn(async () => {
    ...
    iterator: function (fn, ...args) {
      return {
        async *[Symbol.asyncIterator]() {
          ...
          // GH API responses are typically in the shape:
          // { status, url, headers: {...}, data: { datakey: [...], total_count: number }}
          const dataKey = Object.keys(resp.data).find(
            (key) => key !== "total_count"
          );
          yield {
            data: resp.data[dataKey],
            // Use length of data if total_count does not exist
            total_count: resp.data.total_count ?? resp.data[dataKey].length,
          };
        },
      };
    },
